      ...
      {[0, 1, 2].map((i) => (
        <View
          ...
          style={{
            ...
            height: 6,
            borderRadius: 3,
            backgroundColor: i < filled ? t.primary : t.borderLight,
          }}
        />
      ))}
    </View>
...
export const PostCard = memo(function PostCard({
  ...
  const handleConvert = () => {
    confirm({
      ...
      message:
        "Transformar em post do diário? A análise de macros será removida.",
  ...
  return (
    <Card
      style={{ marginHorizontal: SCREEN_PADDING, marginBottom: space.lg }}
      padded={false}
    >
      {/* Header: tipo + tempo */}
      <View
      ...
      >
        ...
        <Text style={[typography.caption, { color: t.textMuted }]}>
          {isLocal ? "Enviando…" : timeAgo(post.created_at)}
        </Text>
      </View>
      ...
      {isLocal && (
        ...
        >
          ...
          <Text style={[typography.caption, { color: t.textMuted }]}>
            Pendente — será enviado quando reconectar
          </Text>
        </View>
      )}
      ...
      {post.type === "meal" && !isLocal && (
        <View
          style={{
            padding: space.lg,
            paddingBottom:
              ai && post.ai_status === "completed" ? space.lg : space.md,
          }}
        >
          {post.ai_status === "pending" && (
            ...
            >
              <AILoader size={30} />
              <Text style={[typography.bodySm, { color: t.textSecondary }]}>
                Analisando com IA…
              </Text>
            </View>
          )}
          {post.ai_status === "completed" && ai && (
            <AuroraBackground
              variant="subtle"
              animated={false}
              style={{ borderRadius: radius.lg, padding: space.md }}
            >
              <View
                style={{
                  flexDirection: "row",
          ...
          {post.ai_status === "skipped" &&
            post.ai_error === "QUOTA_EXCEEDED" && (
              <Text style={[typography.caption, { color: t.textMuted }]}>
                Foto salva. O limite de análises por IA de hoje foi atingido —
                novas análises liberam amanhã.
              </Text>
            )}
          {notFood && (
            <View>
              <Text style={[typography.caption, { color: t.textMuted }]}>
                Não identificamos um alimento nesta foto.
              </Text>
              <Pressable
      ...
      {(post.caption || post.emoji) && (
        <View
          style={{
            ...
            paddingTop: post.type === "meal" && !isLocal ? 0 : space.sm,
          }}
        >
        ...
        >
          {post.reactions.length > 0 && (
            ...
            >
              <Text style={{ fontSize: 15 }}>
                {post.reactions.map((r) => r.emoji).join(" ")}
              </Text>
              <Text style={[typography.caption, { color: t.textSecondary }]}>
                seu nutricionista reagiu
              </Text>
            </View>
          )}
          {post.comments.map((cm) => (
            ...
            >
              ...
              >
                <Text style={[typography.labelSm, { color: t.text }]}>
                  {nutriName}
                </Text>
                <Text style={[typography.bodySm, { color: t.textSecondary }]}>
                  {cm.comment_text}
