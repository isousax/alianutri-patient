...
import { Flame, Award, Star, Target, Trophy, Utensils, Sparkles, Droplets } from 'lucide-react-native'
...
export function Conquistas({ bottomPadding = 100 }: { bottomPadding?: number }) {
  ...
  const pct = maxed ? 1 : Math.max(0, Math.min(1, gam.xpPerLevel > 0 ? gam.xpInLevel / gam.xpPerLevel : 0))
  const cellW = (Dimensions.get('window').width - SCREEN_PADDING * 2 - space.sm * 2) / 3

  return (
    <ScrollView
      style={{ flex: 1 }}
      contentContainerStyle={{ paddingHorizontal: SCREEN_PADDING, paddingTop: space.md, paddingBottom: bottomPadding }}
    ...
    >
        ...
        <Card>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: space.md }}>
            <View style={{ width: 56, height: 56, borderRadius: 28, backgroundColor: t.primary, alignItems: 'center', justifyContent: 'center' }}>
              <Text style={[typography.headingMd, { color: t.primaryFg }]}>{gam.level}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={[typography.caption, { color: t.textMuted }]}>Nível {gam.level}</Text>
              <Text style={[typography.headingMd, { color: t.text }]}>{levelTitle(gam.level)}</Text>
            </View>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: space.sm, paddingVertical: 6, borderRadius: radius.full, backgroundColor: t.warningLight }}>
              <Flame size={15} color={t.warning} />
              <Text style={[typography.captionBold, { color: t.warning }]}>{gam.streak}</Text>
            </View>
          </View>
      ...
      {/* Desafios da semana */}
      <Text style={[typography.labelMd, { color: t.textSecondary, marginTop: space.xl, marginBottom: space.md }]}>
        Desafios da semana
      </Text>
      <Card>
        {challenges.map((ch, i) => {
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: space.sm }}>
        {gam.badges.map((badge) => {
          ...
          return (
            <View
              key={badge.id}
              style={{ width: cellW, alignItems: 'center', opacity: badge.unlocked ? 1 : 0.4 }}
            ...
            >
              ...
              >
                <Icon size={24} color={badge.unlocked ? t.primary : t.textMuted} strokeWidth={2} />
              </View>
              <Text style={[typography.labelSm, { color: t.text, textAlign: 'center' }]} numberOfLines={1}>{badge.label}</Text>
              <Text style={[typography.caption, { color: t.textMuted, textAlign: 'center' }]} numberOfLines={2}>{badge.hint}</Text>
            </View>
          )
        })}
      </View>
