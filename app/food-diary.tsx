...
export default function DiaryScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: t.background }} edges={['top']}>
      ...
      {!isToday && (
        <Text style={[typography.caption, { color: t.info, textAlign: 'center', marginVertical: space.sm }]}>
          Modo retroativo — você está registrando em {fmtDate(date)}.
        </Text>
      )}
...
function FreeDiary({
  date, canWrite, logEntry,
}: {
  date: string; canWrite: boolean;
  logEntry: (entry: {
  ...
  const handleDelete = useCallback(async (id: string) => {
    confirm({
      ...
      destructive: true,
      onConfirm: async () => {
        try { await deleteEntry(id); refetch() } catch { toast.error('Falha ao remover.') }
      },
    })
  }, [deleteEntry, refetch])
            ...
            return (
              ...
              >
                ...
                <View className="flex-1">
                  ...
                  {isRetroactive(entry) && (
                    <Text style={{ color: t.info }} className="text-[10px] font-sans-medium mt-0.5">retroativo</Text>
                  )}
                </View>
                {canWrite && (
...
function CompactLoggedRow({
  meal, isExpanded, onPress, onUndo, isDeleting, canWrite, t,
}: {
  meal: DiaryTimelineMeal
  isExpanded: boolean
  onPress: () => void
    ...
    <View className="mb-1.5">
      ...
      >
        ...
        {meal.entry && isRetroactive(meal.entry) && (
          <View className="px-1.5 py-0.5 rounded ml-1" style={{ backgroundColor: t.infoLight }}>
            <Text className="text-[10px] font-sans-medium" style={{ color: t.info }}>retroativo</Text>
          </View>
        )}
