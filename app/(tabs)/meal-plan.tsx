...
export default function MealPlanScreen() {
  ...
  if (selectedId && detail) {
    const meals: PortalMeal[] = Array.isArray(detail.meals)
      ? (detail.meals as PortalMeal[])
      : [];
    ...
    const isActiveToday =
      !!diaryToday?.meal_plan && diaryToday.meal_plan.id === detail.id;
    const todayMeals = diaryToday?.meals ?? [];
    const loggedToday = todayMeals.filter((m) => m.entry).length;
    const progress =
      todayMeals.length > 0 ? loggedToday / todayMeals.length : 0;
    ...
    return (
      <View style={{ flex: 1, backgroundColor: t.background }}>
        <SafeAreaView style={{ flex: 1 }} edges={["top"]}>
          ...
          <View
            style={{ marginHorizontal: SCREEN_PADDING, marginBottom: space.sm }}
          >
            <View
              style={{
              ...
              >
                <View
                  style={{
                    ...
                    alignItems: "center",
                    gap: 6,
                    paddingHorizontal: space.md, // ou 12
                    paddingVertical: 5, // um pouco mais de altura
                ...
                {meals.length > 0 ? (
                  <Text style={[typography.caption, { color: dm.color }]}>
                    {meals.length}{" "}
              ...
              {detail.total_kcal ? (
                ...
                >
                  ...
                  <Text
                    style={[
                      typography.caption,
                      { color: dm.color, opacity: 0.8, marginBottom: 5 },
                    ]}
                  >
                    kcal / dia
                  </Text>
          ...
          {/*isActiveToday && todayMeals.length > 0 && (
            ...
            >
              ...
              >
                <Text style={[typography.captionBold, { color: t.text }]}>
                  Adesão de hoje
                </Text>
                <Text style={[typography.caption, { color: t.textMuted }]}>
                  {loggedToday}/{todayMeals.length} registradas
                </Text>
              </View>
          ...
          >
            {meals.map((meal, idx) => {
              ...
              return (
                <Animated.View
                  key={idx}
                  entering={FadeInDown.duration(250).delay(idx * 50)}
                >
                  <Card style={{ marginBottom: space.lg, overflow: "hidden" }}>
                    ...
                    >
                      ...
                      >
                        <Text
                          style={[typography.captionBold, { color: dm.color }]}
                        >
                          {idx + 1}
                        </Text>
                      </View>
                    ...
                    {/* Quantitativo */}
                    {detail.method !== "equivalents" &&
                      detail.method !== "qualitative" &&
                      asArray<QuantFood>(qm.foods).map((food, fi) => {
                        const subs = asArray<QuantFoodSubstitute>(
                          food.substitutes,
                        );
                        return (
                          <View
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                • {food.name || food.food_description}
                              </Text>
                              {food.measure ? (
                                <Text
                                  style={[
                    ...
                    {detail.method === "equivalents" &&
                      asArray<EquivGroup>(em.groups).map((g, gi) => (
                        <View
                          key={gi}
                          style={{
                            marginLeft: 28 + space.sm,
                      ...
                      <View style={{ marginLeft: 28 + space.sm }}>
                        {lm.guidance ? (
                          <Text
                            style={[
                              typography.bodySm,
                              { color: t.textSecondary, lineHeight: 20 },
                            ]}
                          >
                            {lm.guidance}
                          </Text>
                        ) : null}
                        {asArray<string | { description: string }>(
                          lm.items,
                        ).map((it, j) => (
                          <Text
                            key={j}
                          ...
                          >
                            • {typeof it === "string" ? it : it.description}
                          </Text>
                        ))}
                      </View>
                    )}
                        ...
                        >
                          ...
                          >
                            <Check
                              size={13}
                              color={
                                status === "partial" ? t.warning : t.primary
                              }
                            />
                            <Text
                              style={[
                            ...
                            >
                              {status === "partial" ? "Parcial" : "Seguida"}
                            </Text>
                          </View>
                          {canWrite && (
                            <Pressable
      ...
      >
        ...
        {plans.length > 1 && (
          ...
          >
            ...
            {olderPlans.length > 0
              ? ` · ${olderPlans.length} anterior${olderPlans.length > 1 ? "es" : ""}`
              : ""}
          </Text>
        )}
      </View>
          ...
          return (
            ...
            >
              <Card
                ...
                style={{ marginBottom: space.lg, marginTop: space.xs}}
              >
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  ...
                  >
                    <NutritionPlan size={70} />
                    {/*<PlanIcon size={18} color={pm.color} />*/}
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text
                      style={[typography.headingSm, { color: t.text }]}
                    ...
                    >
                      ...
                      >
                        <Text
                          style={[typography.overline, { color: pm.color }]}
                        >
                          {plan.total_kcal
                            ? `${plan.total_kcal} kcal/dia`
                            : "Plano alimentar"}
                        </Text>
                      </View>
                    </View>
            ...
            {olderPlans.map((plan, i) => (
              ...
              >
                ...
                >
                  ...
                  >
                    <NutritionPlan size={50} isSubstituted={true} />
                    {/**<Utensils size={14} color={t.textMuted} />*/}
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text
                      style={[typography.labelMd, { color: t.textSecondary }]}
                      numberOfLines={1}
                    >
                      {plan.name}
                    </Text>
